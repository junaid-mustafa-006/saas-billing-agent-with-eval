import tools
from runner import Task, generate_and_freeze_ground_truth, run_suite
...
t_add_seats_happy = Task(
    ...
    seed_sql="",
    correct_sequence=lambda: [tools.add_seats(cus_id=1, num_seats=2, today='2026-06-20')],
...
t_add_seats_cap_refusal = Task(
    ...
    seed_sql="",
    correct_sequence=lambda: [tools.add_seats(cus_id=1, num_seats=15, today='2026-06-20')],
...
t_add_seats_payment_refusal = Task(
    ...
    seed_sql="UPDATE payment_methods SET status = 'expired' WHERE cus_id = 1;",
    correct_sequence=lambda: [tools.add_seats(cus_id=1, num_seats=5, today='2026-06-20')],
...
t_cancel_immediate = Task(
    ...
    seed_sql="",
    correct_sequence=lambda: [tools.cancel_subscription(cus_id=1, today='2026-06-20')],
...
t_cancel_delayed = Task(
    ...
    seed_sql="",
    correct_sequence=lambda: [tools.cancel_subscription(cus_id=2, today='2026-06-20')],
...
t_cancel_anti_abuse = Task(
    ...
    seed_sql="INSERT INTO transactions(cus_id, sub_id, amount_paid, payment_date, type) VALUES(1, 1, 100.0, '2026-05-01', 'refund');",
    correct_sequence=lambda: [tools.cancel_subscription(cus_id=1, today='2026-06-20')],
...
t_cancel_noop = Task(
    ...
    seed_sql="",
    correct_sequence=lambda: [tools.cancel_subscription(cus_id=3, today='2026-06-20')],
...
t_downgrade_overlap = Task(
    ...
    seed_sql="UPDATE subscriptions SET seats_used=13 WHERE cus_id=1; INSERT INTO transactions(cus_id, sub_id, amount_paid, payment_date, type) VALUES(1, 1, 67.5, '2026-06-18', 'seat_add');",
    correct_sequence=lambda: [tools.downgrade_plan(cus_id=1, new_plan_name='Starter', today='2026-06-20', seats=5)],
...
t_downgrade_scheduled = Task(
    ...
    seed_sql="UPDATE subscriptions SET start_date='2026-06-01' WHERE cus_id=1;",
    correct_sequence=lambda: [tools.downgrade_plan(cus_id=1, new_plan_name='Starter', today='2026-06-20', seats=5)],
...
t_upgrade_mid_cycle = Task(
    ...
    seed_sql="",
    correct_sequence=lambda: [tools.upgrade_plan(cus_id=1, new_plan_name='Enterprise', today='2026-06-20', seats=50)],
...
t_upgrade_smart_recovery = Task(
    ...
    seed_sql="",
    correct_sequence=lambda: [tools.downgrade_plan(cus_id=1, new_plan_name='Starter', today='2026-06-20')],
...
t_undo_happy = Task(
    ...
    seed_sql="UPDATE subscriptions SET status='scheduled_downgrade' WHERE cus_id=1; INSERT INTO subscriptions(cus_id, plan_name, seats_used, start_date, end_date, status, autopay) VALUES(1, 'Starter', 5, '2026-07-15', '2026-08-15', 'scheduled_activation', 1);",
    correct_sequence=lambda: [tools.cancel_scheduled_downgrade(cus_id=1)],
...
t_multistep_chain = Task(
    ...
    seed_sql="",
    correct_sequence=lambda: [tools.add_seats(cus_id=1, num_seats=5, today='2026-06-20'), tools.upgrade_plan(cus_id=1, new_plan_name='Enterprise', today='2026-06-20', seats=15)],
...
if __name__ == "__main__":
    # --- PHASE 1: Generate Ground Truths ---
    # Uncomment the two lines below and run once to freeze the test answers
    
    # for t in ALL_TASKS:
    #     generate_and_freeze_ground_truth(t, DB_PATH, SEED_PATH)
    
    # --- PHASE 2: Evaluate the Tasks ---
    run_suite(ALL_TASKS, DB_PATH, SEED_PATH)